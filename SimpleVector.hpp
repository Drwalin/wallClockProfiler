...
template <class Type> class SimpleVector
{
public:
	...
	// add x to the front of the vector (slower)
	...
	Type *getLastElement() { return getElement(size() - 1); }

	Type getLastElementDirect() { return getElementDirect(size() - 1); }
	...
	void deleteLastElement() { deleteElement(size() - 1); }
	 ...
	 *   Defaults to "unnamed".
	 */
	void setPrintMessageOnVectorExpansion(char inPrintMessage,
										  const char *inVectorName = "unnamed");
...
template <class Type> inline SimpleVector<Type>::SimpleVector() : vectorName("")
{
...
template <class Type>
inline SimpleVector<Type>::SimpleVector(int sizeEstimate) : vectorName("")
{
...
template <class Type> inline SimpleVector<Type>::~SimpleVector()
{
	delete[] elements;
}
...
inline SimpleVector<Type>::SimpleVector(const SimpleVector<Type> &inCopy)
	: elements(new Type[inCopy.maxSize]),
	  numFilledElements(inCopy.numFilledElements), maxSize(inCopy.maxSize),
	  minSize(inCopy.minSize),
...
// assignment operator
template <class Type>
inline SimpleVector<Type> &
SimpleVector<Type>::operator=(const SimpleVector<Type> &inOther)
{
...
template <class Type> inline int SimpleVector<Type>::size()
{
	return numFilledElements;
}

template <class Type> inline Type *SimpleVector<Type>::getElement(int index)
{
...
template <class Type> inline Type *SimpleVector<Type>::getElementFast(int index)
{
	return &(elements[index]);
}
...
template <class Type> inline bool SimpleVector<Type>::deleteElement(int index)
{
...
template <>
inline bool SimpleVector<unsigned char>::deleteStartElements(int inNumToDelete)
{
...
template <>
inline bool SimpleVector<char>::deleteStartElements(int inNumToDelete)
{
...
template <class Type> inline void SimpleVector<Type>::shrink(int inNewSize)
{
	numFilledElements = inNewSize;
}

template <class Type> inline void SimpleVector<Type>::swap(int inA, int inB)
{
	if (inA == inB) {
		return;
	}

	if (inA < numFilledElements && inA >= 0 && inB < numFilledElements &&
		inB >= 0) {
...
template <class Type> inline void SimpleVector<Type>::deleteAll()
{
	numFilledElements = 0;
...
template <class Type> inline void SimpleVector<Type>::push_back(Type x)
{
...
template <class Type> inline void SimpleVector<Type>::push_front(Type x)
{
	push_middle(x, 0);
}
...
template <class Type>
inline void
SimpleVector<Type>::push_back_other(SimpleVector<Type> *inOtherVector)
{
...
template <class Type> inline Type *SimpleVector<Type>::getElementArray()
{
...
template <> inline char *SimpleVector<char>::getElementString()
{
...
template <class Type>
inline void
SimpleVector<Type>::setPrintMessageOnVectorExpansion(char inPrintMessage,
													 const char *inVectorName)
{
...
template <> inline void SimpleVector<char *>::deallocateStringElements()
{
	for (int i = 0; i < numFilledElements; i++) {
		delete[] elements[i];
	}
